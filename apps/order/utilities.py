...
def checkout(request, first_name, last_name, email, address, zipcode, place, phone):
    order = Order.objects.create(first_name=first_name, last_name=last_name, email=email, address=address, zipcode=zipcode, place=place, phone=phone)
...
def notify_seller(order):
    ...
    for vendor in order.sellers.all():
        to_email = vendor.created_by.email
        subject = 'New order'
        text_content = 'You have a new order!'
        html_content = render_to_string('email_notify_seller.html', {'order': order, 'seller': vendor})
